...
// Exercise 8-4: The standard library funtction fseek is identical to _lseek
//  except that fp is a file pointer instead of a file descriptor and the
//  return value is an int status, not a position. Write fseek. Make sure that
//  your fseek coordinates properly with the buffering done for the other
...
int fileseek(FILE* fp, long offset, int origin) {
  // Handle invalid file pointer
  if (fp < _iob || fp >= _iob + OPEN_MAX) return -1;
  // Handle invalid file mode
  if (fp->flags._READ == 0 | fp->flags._WRITE == 0) return -1;

  int res = 0;

  if (fp->flags._WRITE == 1) {
    // First flush buffer to file if it contains anything
    int n = fp->ptr - fp->base;
    if (n > 0) {
      if (write(fp->fd, fp->base, n) != n) {
        // Set error if write failed
        res = -1;
      }
    }

    // If we haven't encountered an error, set position in file
    if (res != -1) {
      res = lseek(fp->fd, offset, origin);
    }
  } else if (fp->flags._READ == 1) {
    // Set position in buffer based on origin and offset
    if (origin == SEEK_CUR) offset -= fp->cnt;

    // Seek in file
    res = lseek(fp->fd, offset, origin);
    // Set buffer count to empty
    fp->cnt = 0;
  }
